// src/components/eduAI-related/counselorAI/CounselorLinkifieedText.tsx
import React, { useMemo } from 'react';
import { Alert, Linking, Text } from 'react-native';
import * as WebBrowser from 'expo-web-browser';

export default function CounselorLinkifiedText({
  text,
  ...
  const open = async (u: string) => {
    try {
      ...
      try {
        (await Linking.canOpenURL(u)) && (await Linking.openURL(u));
      } catch {
        Alert.alert('リンクを開けませんでした', u);
      }
    }
  };

...
import { TextInput, Button, HelperText, Snackbar, RadioButton } from 'react-native-paper';
...
export default function RoomCreateForm({ onClose }: Props) {
  ...
  const handleCreate = async () => {
    ...
    // エラーがオブジェクトの時は深掘り
    if (e?.code)   console.error('[RoomCreate] error code:', e.code);
    if (e?.message) console.error('[RoomCreate] error message:', e.message);
    if (typeof e === 'object') console.error('[RoomCreate] error JSON:', JSON.stringify(e));
    }
    setLoading(false);
    console.log('[RoomCreate] END');
  };
  ...
  <View className="flex-row mb-3 border rounded-md p-2 bg-white dark:bg-neutral-900">
    {ROOM_TAGS.map((tag) => (
      ...
      >
        <View
          className={`mr-1 rounded-full border-2 ${
            roomTag === tag.value
              ? 'border-blue-500'
              : 'border-gray-400'
          } p-0.5`}
